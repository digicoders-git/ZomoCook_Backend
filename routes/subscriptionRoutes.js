    ...
    try {
        ...
        if (subHistories && subHistories.length > 0) {
            ...
            subs.push({
                ...
                start_date: new Date(expiry.getTime() - (plan.durationDays * 24 * 3600 * 1000)).toISOString(),
                end_date: user.planExpiryDate,
                joining_date: new Date(expiry.getTime() - (plan.durationDays * 24 * 3600 * 1000)).toISOString(),
                replacement_left: plan.replacementLimit || (user.currentHiringLimit - user.cooksHiredInCurrentPlan) || 0,
                hiring_limit: plan.hiringLimit || user.currentJobPostLimit || 1,
                job_post_limit: plan.jobPostLimit || user.currentJobPostLimit || 1,
                features: plan.features || [],
                applied_count: appliedCount,
                shortlisted_count: shortlistedCount,
                selected_count: selectedCount,
